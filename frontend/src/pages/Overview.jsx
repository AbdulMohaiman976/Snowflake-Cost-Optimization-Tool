    ...
    return (
        ...
        >
            <div className="flex items-end justify-between">
                <div>
                  <h1 className="text-4xl font-extrabold tracking-tight text-text mb-2">Account Overview</h1>
            ...
            <div className="grid grid-cols-3 gap-6">
                    ...
                    <AnimatePresence mode="wait">
                        {selectedTab === 'health' && (
                            <motion.div key="health" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                <div className="mb-0">
                                    <h3 className="text-xl font-bold text-text mb-2 underline decoration-primary/30 underline-offset-8">Account Vitals Analysis</h3>
                                 ...
                                 <div className="grid grid-cols-2 gap-8">
                                     <div className="space-y-4">
                                         <p className="text-[10px] font-black text-text/40 uppercase tracking-[0.2em]">Postures</p>
                                         <div className="space-y-4">
                                             {[
                                     ...
                                     <div className="p-5 bg-black/[0.03] border border-black/10 rounded-2xl relative overflow-hidden">
                                         <div className="absolute top-0 right-0 p-3 opacity-10"><Target className="w-12 h-12 text-primary" /></div>
                                         <p className="text-[10px] font-black text-primary uppercase tracking-widest mb-4">Roadmap to 100</p>
                                         <div className="space-y-3">
                                             {[
                                                 ...
                                                 <div key={i} className="flex items-center gap-3">
                                                     <div className={`w-4 h-4 rounded border flex items-center justify-center ${item.done ? 'bg-success border-success' : 'border-black/20'}`}>
                                                         {item.done && <CheckCircle className="w-3 h-3 text-text" />}
                                                     </div>
                                                     <span className={`text-[11px] font-medium ${item.done ? 'text-text-muted line-through' : 'text-text'}`}>{item.task}</span>
                                                 </div>
                                             ))}
                                         </div>
                                     </div>
                        ...
                        {selectedTab === 'spend' && (
                            <motion.div key="spend" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                <div className="flex items-center justify-between mb-8">
                                    <div>
                                        <h3 className="text-xl font-bold text-text mb-1">Credit Utilization Details</h3>
                                    ...
                                    {chartData.length > 0 ? (
                                        <ResponsiveContainer width="100%" height="100%">
                                            <AreaChart data={chartData}>
                                                ...
                                                <Tooltip contentStyle={{ backgroundColor: '#B7ECF9', border: '1px solid #1a2e4a', borderRadius: '12px' }} />
                                                <Area type="monotone" dataKey="credits" stroke="#2563eb" strokeWidth={3} fill="url(#colorCredits)" />
                                            </AreaChart>
                                        </ResponsiveContainer>
                                    ) : (
                                        <div className="flex flex-col items-center justify-center h-full border border-dashed border-black/10 rounded-2xl bg-black/[0.02]">
                        ...
                        {selectedTab === 'savings' && (
                            <motion.div key="savings" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
                                <h3 className="text-xl font-bold text-text">Savings Opportunities</h3>
                                <div className="grid grid-cols-2 gap-4">
                                    {(savings?.recommendations || savings?.items || []).slice(0, 4).map((rec, i) => (
                                        <div key={i} className="p-4 bg-black/5 border border-black/10 rounded-xl">
                                            <div className="flex items-center gap-2 mb-2">
                                                ...
                                                <span className="text-xs font-bold text-text">{rec.title}</span>
                                            </div>
                        ...
                        {selectedTab === 'alerts' && (
                            <motion.div key="alerts" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
                                <h3 className="text-xl font-bold text-text">Critical Performance Alerts</h3>
                                <div className="space-y-3">
                                    {session?.anomaly?.recent_anomalies?.length > 0 ? (
                                        session.anomaly.recent_anomalies.slice(0, 3).map((alert, i) => (
                                            <div key={i} className="flex items-center justify-between p-4 bg-black/[0.02] border border-border rounded-xl group hover:border-danger/30 transition-all">
                                                <div className="flex items-center gap-4">
                                                    <div className="p-2 bg-danger/10 border border-danger/20 rounded-lg">
                                                        <AlertCircle className="w-5 h-5 text-danger" />
                                                    </div>
                                                    <div className="flex flex-col">
                                                        <span className="text-sm font-bold text-text">{alert.warehouse}</span>
                                                ...
                                                <div className="text-right flex items-center gap-4">
                                                    <div>
                                                        ...
                                                        <p className="text-xs font-black text-text">+{alert.magnitude?.toFixed(0)}% Credit Spike</p>
                                                    </div>
                ...
                <div className="glass-card p-8 flex flex-col">
                    <div className="mb-6">
                      <h3 className="text-lg font-bold text-text mb-1">AI Recommendation</h3>
                    ...
                    <div className="flex-1 flex flex-col justify-start items-center text-center p-6 bg-primary/5 border border-dashed border-primary/20 rounded-2xl min-h-[300px]">
                        ...
                        <div className="text-sm font-medium text-text leading-relaxed mb-6 text-left w-full h-full max-h-[400px] overflow-y-auto whitespace-pre-wrap">
                            {isGenerating ? <span className="animate-pulse text-primary">Generating real-time SQL architecture strategy via Groq API...</span> : aiRecs}
                        </div>
                        <button 
                            onClick={handleExecuteAdvice}
                            disabled={isGenerating}
                            className={`px-8 py-3 w-full bg-primary text-text text-xs font-bold rounded-xl transition-all mt-auto ${isGenerating ? 'opacity-50 cursor-not-allowed' : 'hover:bg-primary-dark'}`}
                        >
                        ...
                        <div className="space-y-4">
                            {[
                                ...
                                <div key={dim.name} className="flex items-center gap-3">
                                    ...
                                    <span className="text-[10px] font-bold text-text min-w-[24px]">{dim.score}%</span>
                                </div>
                            ))}
                        </div>
                    </div>

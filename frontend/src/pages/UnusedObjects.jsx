    ...
    return (
        ...
        >
            <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-3xl font-extrabold tracking-tight text-text mb-2">Unused Objects</h1>
                ...
                <div className="flex gap-4 p-4 bg-black/30 border border-border rounded-xl">
                  ...
                  <div className="pl-2">
                    ...
                    <p className="text-xl font-black text-text font-mono">{totalSize?.toFixed(2)} GB</p>
                  </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-4 gap-6">
                <div className="p-6 glass-card bg-gradient-to-br from-sidebar/80 to-background flex flex-col justify-between h-44">
                    <div>
                        ...
                        <h4 className="text-2xl font-extrabold text-text">{tables.length}</h4>
                    </div>
                ...
                <div className="p-6 glass-card bg-gradient-to-br from-sidebar/80 to-background flex flex-col justify-between h-44 border-l-4 border-primary">
                    <div>
                        ...
                        <h4 className="text-2xl font-extrabold text-text">{tables.length > 0 ? tables[0].size_gb : 0} GB</h4>
                    </div>
                    <div className="text-[10px] text-text-muted font-bold flex items-center gap-1.5 uppercase tracking-widest">
                ...
                <div className="p-6 glass-card bg-gradient-to-br from-sidebar/80 to-background flex flex-col justify-between h-44 backdrop-blur-3xl overflow-hidden relative group">
                  <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-15 transition-opacity"><Layers className="w-20 h-20 text-text" /></div>
                    <div>
                        <p className="text-[10px] text-primary font-black uppercase tracking-[0.2em] mb-4 leading-none">AI Priority</p>
                        <h4 className="text-xl font-bold text-text line-clamp-1">{tables.length > 10 ? 'High Cleanup' : 'Regular Audit'}</h4>
                    </div>
                    <div className="w-full h-8 flex items-center justify-center border border-primary/20 rounded-lg text-[10px] font-bold text-primary uppercase tracking-widest">
            ...
            <div className="glass-card overflow-hidden">
                <div className="p-6 border-b border-border bg-sidebar/30 flex items-center justify-between">
                  <h3 className="text-sm font-bold text-text uppercase tracking-widest">Candidate Tables for Deletion</h3>
                  <div className="flex gap-2">
                    <div className="relative">
                        <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-text-muted" />
                        <input className="bg-sidebar-item border border-border rounded-lg py-1.5 pl-8 pr-3 text-[11px] text-text focus:outline-none focus:border-primary/50 transition-all w-[240px]" placeholder="Search stale schemas..." />
                    </div>
                    <button className="p-1 px-3 rounded-lg hover:bg-black/5 border border-border text-text-muted font-black text-[10px] uppercase">Filter Age</button>
                  </div>
                </div>

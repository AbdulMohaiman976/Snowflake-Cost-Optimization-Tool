    ...
    return (
        ...
        >
            <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-3xl font-extrabold tracking-tight text-white mb-2">Savings & History</h1>
                ...
                <div className="flex gap-4 p-4 bg-sidebar/50 border border-border rounded-xl">
                  <div className="pr-6 border-r border-border/50 text-right">
                    ...
                    <p className="text-xl font-black text-white font-mono">${(savings.total_usd || 0).toLocaleString()}/mo</p>
                  </div>
                  <div className="pl-2 text-right">
                    <p className="text-[10px] text-text-muted font-bold uppercase tracking-widest mb-1.5 leading-none">Last Analyzed</p>
                    <p className="text-sm font-black text-primary-light flex items-center justify-end gap-1.5 mt-1.5"><Calendar className="w-4 h-4" /> Today</p>
                  </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6">
                <div className="glass-card p-8 flex flex-col justify-between border-l-4 border-success overflow-hidden relative group">
                    ...
                    <div>
                        <h3 className="text-sm font-bold text-white uppercase tracking-widest mb-2">Quick Optimization Action</h3>
                        ...
                        <div className="flex gap-4">
                            <button className="flex-1 py-3 bg-success hover:bg-success-dark text-white text-[10px] font-black uppercase rounded-xl transition-all shadow-lg shadow-success/20">Apply All Best Practices</button>
                            <button className="flex-1 py-3 bg-white/5 hover:bg-white/10 border border-white/10 text-white text-[10px] font-black uppercase rounded-xl transition-all">Review Pipeline</button>
                        </div>
                    </div>
                </div>
                ...
                <div className="glass-card p-8 flex flex-col justify-between border-l-4 border-primary">
                    <div>
                        <h3 className="text-sm font-bold text-white uppercase tracking-widest mb-6">Session Execution History</h3>
                        <div className="space-y-4">
                            {[
                                ...
                                <div key={idx} className="flex items-center justify-between p-3 bg-sidebar/50 border border-border rounded-xl group hover:border-primary/50 transition-colors cursor-pointer">
                                    <div className="flex items-center gap-4">
                                        <div className="p-2 bg-primary/10 border border-primary/20 rounded-lg"><History className="w-3.5 h-3.5 text-primary-light" /></div>
                                        <div className="flex flex-col">
                                            <span className="text-xs font-bold text-white">{run.date} Check</span>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between px-2">
                    <h3 className="text-sm font-bold text-white uppercase tracking-widest">Master Recommendations List</h3>
                ...
                <div className="grid grid-cols-1 gap-4">
                    {items.map((rec, idx) => (
                        <div key={idx} className={`p-6 glass-card border-l-[4px] bg-gradient-to-br from-sidebar/50 to-background flex flex-col gap-6 group hover:border-white/20 transition-all ${rec.severity === 'HIGH' ? 'border-danger' : rec.severity === 'MEDIUM' ? 'border-warning' : 'border-primary'}`}>
                            <div className="flex items-start justify-between">
                                <div className="flex gap-4">
                                    <div className={`p-3 rounded-2xl border ${rec.severity === 'HIGH' ? 'bg-danger/10 border-danger/20 text-danger' : rec.severity === 'MEDIUM' ? 'bg-warning/10 border-warning/20 text-warning' : 'bg-primary/10 border-primary/20 text-primary-light'}`}>
                                ...
                                <div className="flex flex-col items-end gap-3">
                                   <div className={`p-1 px-3 rounded-md text-[10px] font-black uppercase text-white ${rec.severity === 'HIGH' ? 'bg-danger' : rec.severity === 'MEDIUM' ? 'bg-warning text-black' : 'bg-primary'}`}>
                                       {rec.severity} SEVERITY
                                   </div>
                                    <span className="text-[10px] text-text-muted font-bold flex items-center gap-1.5 uppercase tracking-widest cursor-pointer hover:text-white transition-colors underline underline-offset-4">Read full analysis <ArrowUpRight className="w-3 h-3" /></span>
                                </div>
                            </div>
                            
                            {rec.fix_sql && (
                                <div className="bg-[#0b1220] rounded-xl border border-border/40 p-5 font-mono text-xs text-text-accent leading-relaxed relative flex flex-col group/code">
                                    <div className="flex items-center justify-between mb-4 pb-4 border-b border-white/5 opacity-40 group-hover/code:opacity-100 transition-opacity">
                                       <span className="text-[9px] font-black uppercase tracking-[0.2em] text-primary-light">Proposed Resolution SQL</span>
                                       <button className="flex items-center gap-1.5 p-1 px-2 border border-primary/20 bg-primary/20 text-[9px] font-bold text-primary-light uppercase rounded transition-colors hover:bg-primary hover:text-white">Copy Command</button>
                                    </div>

    ...
    return (
        ...
        >
            <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-3xl font-extrabold tracking-tight text-text mb-2">Cloud Services</h1>
                ...
                <div className="flex gap-4 p-4 bg-black/30 border border-border rounded-xl backdrop-blur-xl">
                  <div className="pr-6 border-r border-border/50">
                    ...
                    <p className="text-xl font-black text-text font-mono">{totalCloud?.toFixed(4)}</p>
                  </div>
                  <div className="pl-2">
                    <p className="text-[10px] text-text-muted font-bold uppercase tracking-widest mb-1.5 leading-none">Market Price (USD)</p>
                    <p className="text-xl font-black text-primary font-mono">${(totalCloud * 3).toLocaleString()}</p>
                  </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-4 gap-6 mb-8">
                <div className="p-6 glass-card bg-gradient-to-br from-sidebar/80 to-background flex flex-col justify-between h-44">
                    <div>
                        ...
                        <h4 className="text-2xl font-extrabold text-text">
                ...
                <div className="p-6 glass-card bg-gradient-to-br from-sidebar/80 to-background flex flex-col justify-between h-44 border-l-4 border-warning">
                    <div>
                        ...
                        <h4 className="text-2xl font-extrabold text-text">{warehouses.length} clusters</h4>
                    </div>
                ...
                <div className="col-span-2 p-6 glass-card border border-primary/20 bg-primary/5 relative overflow-hidden group hover:border-primary/40 transition-colors cursor-pointer">
                  <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity"><Zap className="w-24 h-24 text-primary" /></div>
                    <div>
                        <p className="text-[10px] text-primary font-black uppercase tracking-[0.2em] mb-4 leading-none">Cloud Strategy Recommendation</p>
                        <h4 className="text-lg font-bold text-text mb-2 max-w-sm leading-tight font-mono">
                            {warehouses.length > 0 
                    ...
                    <div className="mt-4 flex items-center gap-3">
                        <button className="py-1.5 px-4 bg-primary text-text text-[10px] font-black uppercase rounded-lg transition-all shadow-md">Run Deep Audit</button>
                    </div>
                </div>
            </div>
            ...
            <div className="glass-card overflow-hidden">
                <div className="p-6 border-b border-border bg-sidebar/30 flex items-center justify-between">
                  <h3 className="text-sm font-bold text-text uppercase tracking-widest">Warehouse Level Cloud Performance</h3>
                  <div className="flex gap-2">
                    <div className="relative">
                        <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-text-muted" />
                        <input className="bg-sidebar-item border border-border rounded-lg py-1.5 pl-8 pr-3 text-[11px] text-text focus:outline-none focus:border-primary/50 transition-all w-[240px]" placeholder="Search warehouse..." />
                    </div>
                  </div>
                </div>
                                ...
                                <tr key={idx} className="group hover:bg-black/[0.02] transition-colors">
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-xs font-bold text-text group-hover:text-primary transition-colors">{wh.warehouse}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 text-right font-mono text-[11px] text-text font-bold">{wh.total_credits?.toFixed(4)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className={`text-[10px] font-bold ${wh.cloud_pct > 10 ? 'text-primary' : 'text-text-muted'}`}>{wh.cloud_pct}%</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 text-right">
                                        <button className="group flex items-center justify-end gap-1.5 p-1 px-3 text-[10px] font-black uppercase text-primary transition-colors hover:text-text">Profile <ArrowUpRight className="w-3 h-3 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" /></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
